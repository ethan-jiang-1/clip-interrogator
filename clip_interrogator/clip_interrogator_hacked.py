...
try:
    from models.blip import blip_decoder  # noqa 
except: # noqa:
...
# this is v1
DIR_ROOT = None 
if os.path.isdir("/root/autodl-nas"):
    from main_root import add_prj_root_to_sys_path
    DIR_ROOT = add_prj_root_to_sys_path()

    from main.loggers.logger_main import get_logger_main
    g_logger = get_logger_main(need_stdout=True)
else:
...
class Config:
    ...
    if DIR_ROOT is None:
        ...
        clip_model_path: str = f"{DIR_ROOT}/experiments/openai"
...
class InterrogatorBase:
    def __init__(self, config: Config, logger: logging.Logger = g_logger):
        ...
        self._load_blip_model(config)
        if config.ci_mode.find("clip") != -1:
            self._load_clip_model(config)
        self.logger.info(f"Loaded ci_mode: {self.config.ci_mode}")
        if self.blip_model is None:
            raise ValueError("blip model is None")

    def _load_blip_model(self, config: Config):
        start_time = time.time()

        if config.blip_model is None:
            if not config.quiet:
                self.logger.info(f"Loading BLIP model {config.blip_model_url} ...")
        ...
        end_time = time.time()
        if not config.quiet:
            self.logger.info(f"Loading BLIP model in {end_time-start_time:.2f} seconds.")

    def _load_clip_model(self, config: Config):
        start_time = time.time()

        if config.clip_model is None:
            if not config.quiet:
                self.logger.info(f"Loading CLIP model {config.clip_model_name}...")
            ...
            if not config.quiet:
                self.logger.info("Loading CLIP done...")
        else:
    ...
    def interrogate_caption(self, image: Image) -> str:
...
class InterrogatorOrg(InterrogatorBase):
    def __init__(self, config: Config, logger: logging.Logger = None):
        super(InterrogatorOrg, self).__init__(config, logger=logger)
        self._build_ci_org_label_list(config)

    def _build_ci_org_label_list(self, config:Config):
        ...
        if not config.quiet:
            self.logger.info(f"Loading CLIP label org data model {config.clip_model_name}...")
        ...
        end_time = time.time()
        if not config.quiet:
            self.logger.info(f"Loaded CLIP label org data in {end_time-start_time:.2f} seconds.")
...
class InterrogatorPet(InterrogatorBase):
    ...
    def _build_ci_pet_label_list(self, config:Config): 
        ...
        if not config.quiet:
            self.logger.info(f"Loading CLIP label pet data model {config.clip_model_name}...")

        names = os.listdir(config.data_path)
        names_txt = []
        for name in names:
            fname = os.path.join(config.data_path, name)
            if name.startswith("pet_") and name.endswith(".txt"):
                names_txt.append(fname)

        names_txt = sorted(names_txt)
        
        for name in names_txt:
            label_list = _load_list(config.data_path, name)
            if len(label_list) > 0:
                label_name = os.path.basename(name).split(".")[0]
                label_table = LabelTable(label_list, label_name, self.clip_model, self.tokenize, config)
                self.pet_lablels.append(label_table)
